export const votaciones = [
  {
    id: 1,
    titulo: "Votación de Comida",
    descripcion: "Decide el menú para el evento de fin de año",
    fechaFin: "2025-11-10",
    estado: "activa",
    badge: "Activa",
    image: "https://img.freepik.com/vector-gratis/kawaii-comida-rapida-lindo-hot-dog-comida-rapida-hamburguesas-papas-fritas-bebida-ilustracion-salsa-tomate_24908-60601.jpg?semt=ais_hybrid&w=740&q=80",
    candidatos: [
      { id: 1, nombre: "Pizza", votos: 14, foto: "https://cdn-icons-png.flaticon.com/512/1046/1046784.png" },
      { id: 2, nombre: "Hamburguesa", votos: 26, foto: "https://cdn-icons-png.flaticon.com/512/3075/3075977.png" },
      { id: 3, nombre: "Pollo a la brasa", votos: 50, foto: "https://cdn-icons-png.flaticon.com/512/135/135763.png" }
    ]
  },
  {
    id: 2,
    titulo: "Mejor Lugar Turistico del Mes",
    descripcion: "Vota por el mejor lugar turistico de nuestro pais",
    fechaFin: "2025-11-01",
    estado: "activa",
    badge: "Activa",
    image: "https://travel1tours.com/wp-content/uploads/2019/09/Lugares-turisticos-de-Peru.jpg",
    candidatos: [
      { id: 1, nombre: "Machu Picchu", votos: 100, foto: "https://cdn-icons-png.flaticon.com/512/684/684908.png" },
      { id: 2, nombre: "Museo de arte de Lima", votos: 36, foto: "https://cdn-icons-png.flaticon.com/512/190/190602.png" },
      { id: 3, nombre: "Castillo de Chancay", votos: 20, foto: "https://cdn-icons-png.flaticon.com/512/684/684908.png" }
    ]
  },
  {
    id: 3,
    titulo: "Proyecto Prioritario Q4",
    descripcion: "Selección de proyectos para el cuarto trimestre",
    fechaFin: "2025-10-15",
    estado: "cerrada",
    badge: "Cerrada",
    image: "https://www.codemotion.com/magazine/wp-content/uploads/2023/02/shahadat-rahman-gnyA8vd3Otc-unsplash-896x504.jpg",
    candidatos: [
      { id: 1, nombre: "Crear un juego de adivinar un numero", votos: 23, foto: "" },
      { id: 2, nombre: "Realizar una pagina Web de votación", votos: 26, foto: "" }
    ]
  },
  {
    id: 4,
    titulo: "Horario de Oficina",
    descripcion: "Votación sobre el horario flexible",
    fechaFin: "2025-09-20",
    estado: "cerrada",
    badge: "Cerrada",
    image: "https://pensamientocarlosreyes.com/wp-content/uploads/2016/04/personal-885550_640_mini.jpg?w=640",
    candidatos: [
      { id: 1, nombre: "8am - 5pm", votos: 3, foto: "" },
      { id: 2, nombre: "9am - 6pm", votos: 10, foto: "" }
    ]
  },
  {
    id: 5,
    titulo: "Beneficios del Equipo",
    descripcion: "Selecciona los beneficios preferidos",
    fechaFin: "2025-11-05",
    estado: "activa",
    badge: "Activa",
    image: "https://coolspaces.mx/wp-content/uploads/2019/03/53910532_365473977375006_5738127318782574592_n.jpg",
    candidatos: [
      { id: 1, nombre: "Snacks gratis", votos: 10, foto: "" },
      { id: 2, nombre: "Día de home office", votos: 15, foto: "" },
      { id: 3, nombre: "Bono mensual", votos: 40, foto: "" }
    ]
  }
];
